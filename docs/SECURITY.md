...
| ML proxy scope binding (`org_id`/`manager_id`) | A caller in possession of `X-Internal-Api-Key` requesting another org's/manager's data by changing a query param | Backend signs an `X-Internal-Claims` JWT (RS256) asserting `org_id`/`manager_id` per request (`MlClaimsSigner`); FastAPI verifies it with a public key it never holds the private half of (see ADR-0008). Compromising the ML container's environment does not grant the ability to forge a different scope. | Verification falls back to trusting the raw query params when `INTERNAL_CLAIMS_PUBLIC_KEY` is empty (local dev only) — must be set whenever the ML service is reachable outside the backend's trusted network, same as `INTERNAL_API_KEY` |
